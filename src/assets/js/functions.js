...
const systemFunction = {
  ...
  interactiveData: function (_this, businessType, handleType, data, dataType, callback) {
    ...
    _this.FUNCTIONS.systemFunction.jsonPost(
      ...
      resultData => {
        ...
        } else if (resultData.code === '1') {//操作失败，提示异常信息
          _this.$message({
            showClose: true,
  ...
  fileDownload: function (file) {
    (file && file.response &&
      (file.response.code === '0') && file.response.data.fileUrl) ?
  ...
  fileLimit: function () {
    this.$message.warning("附件最多可以上传5个");
  },
  fileBeforeUpload: function (file) {
    let fileType = ['.zip','.7z','.rar','.ppt','.pptx','.pptm','.pdf','.doc','.docx','.docm','.xls','.xlsx','.xlsm','.image','.jpg','.jpeg','.png','.gif','.gpj']
    let txt = file.name.substring(file.name.lastIndexOf('.'))
    let fileTypeFlag = fileType.find(function(value) {
      if(value === txt) {
        return true
      }
     })
    ...
    if (file.size > 100 * 1024 * 1024) {
      this.$message.warning('文件超过100M，请分包上传')
  ...
  //校验金额必填项是否大于0
  checkMoneyNum: (rule, value, callback) => {
    if (!value) {
  ...
  //自动生成流程编号
  createFlowNum: function () {
    const now = new Date()
    let year = now.getFullYear()
    let month = now.getMonth() + 1
    let day = now.getDate()
    return 'FLSW-' + year.toString() + month.toString() + day + this.randomNum(5).toString()
  },
