  ...
  export default {
    ...
    data () {
      return {
        ...
        sourceList: JSON.parse(unescape(localStorage.getItem(this.GLOBAL.config.sourceList))) ?
          JSON.parse(unescape(localStorage.getItem(this.GLOBAL.config.sourceList))).filter(item => !item.value.indexOf(this.GLOBAL.config.modeName.cases))
          :
          null
      }
    },
    ...
    methods: {
      ...
      hasSource: function (targetValue) {
        let sourceInfo = this.sourceList ? this.sourceList.filter(item => item.value.indexOf(targetValue) >= 0) : []
