<template>
  <div class="main-area">
    <el-page-header @back="goBack" content="诉讼类案件编辑"></el-page-header>
    <el-form
      :inline="true"
      :model="formData"
      class="demo-ruleForm"
    ...
    >
      ...
      <el-row v-for="(item, index) in formData.hearInformationList" :key="item.id ? item.id : index">
        ...
        <hearInformation :info-data="item" :case-status="formData.caseStatus" :key="Date.now()" ref="hearInformation"/>
        <el-divider></el-divider>
      </el-row>
      <el-row>
        <el-col :span="8">
          <el-form-item label="回款/减损金：" prop="derogationAmount"
                        :rules="caseStatusClosed?formRules.derogationAmount:[{required: false, message: '请输入回款/减损金', trigger: 'blur'}]">
            <el-input-number
        ...
        <el-col :span="8">
          <el-form-item label="结案方式：" prop="closingMethod"
                        :rules="caseStatusClosed?formRules.closingMethod:[{required: false, message: '请选择结案方式', trigger: 'blur'}]">
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="案件小结：" prop="summary"
                        :rules="caseStatusClosed?formRules.summary:[{required: false, message: '请输入案件小结', trigger: 'blur'}]">
            <el-input
              type="textarea"
              autosize
              placeholder="案件小结"
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="案件复盘：" prop="replay"
                        :rules="caseStatusClosed?formRules.replay:[{required: false, message: '请输入案件复盘', trigger: 'blur'}]">
            <el-input
              type="textarea"
              autosize
              placeholder="案件复盘"
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        formRules: {
          ...
          closingMethod: [
            {required: true, message: '请设置结案方式', trigger: 'blur'}
          ],
          summary: [
    ...
    methods: {
      ...
      saveForm: function () {
        //页面必填校验
        let editValid = this.editCasePageValid()
        if (editValid) {
          ...
          for (let i = 0; i < this.formData.hearInformationList.length; i++) {
            if (this.$refs.hearInformation[i].getData('validate')) {
              ...
              this.formData.hearInformationList[i].sort = i+1
                for(let n = 0;n<this.formData.hearInformationList[i].caseProcess.length;n++){
                    let content = this.formData.hearInformationList[i].caseProcess[n].content
                    if(content && content.trim()){
                        this.formData.hearInformationList[i].caseProcess[n].sort = n
                    }
                }
            } else {
              return
      ...
      editCasePageValid: function () {
        ...
        //主页面校验====end===========
        //结案：子页面校验====start===========
        let subValidate = true
        if (this.caseStatusClosed) {
          for (let i = 0; i < this.formData.hearInformationList.length; i++) {
            // 结案状态，校验审理信息必填项
            this.$refs.hearInformation[i].$refs.formData.validate(valid => {
              if (!valid) {
                subValidate = false
              }
            })
