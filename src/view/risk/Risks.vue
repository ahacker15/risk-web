<template>
  <div class="main-area">
    <el-breadcrumb separator=":">
      ...
      <el-breadcrumb-item>风险信息</el-breadcrumb-item>
    </el-breadcrumb>
    <!--查询区域-->
    ...
    <el-row class="margin-top-20">
      ...
      <el-col :span="5">
        <el-date-picker
          ...
          v-model="searchForm.registrateTime"
          type="daterange"
          align="right"
          unlink-panels
          range-separator="至"
      ...
      <el-col :span="3">
        <el-select v-model="searchForm.riskType"
                   placeholder="风险类型"
                   :size="GLOBAL.config.systemSize"
                   style="width: 100%;"
        >
      ...
      <el-col :span="3">
        <el-input v-model="searchForm.legalEntity"
    ...
    <el-row class="margin-top-10">
      ...
      <el-col :span="3">
        <el-select v-model="searchForm.riskGrade"
                   placeholder="风险等级"
                   :size="GLOBAL.config.systemSize"
                   style="width: 100%;"
        >
      ...
      <el-col :span="4">
        ...
        <el-button
          v-if="hasSource(GLOBAL.config.sourceName.emport)"
          type="primary" @click="exportTableData" :size="GLOBAL.config.systemSize" icon="el-icon-download">导出
        </el-button>
      </el-col>
    </el-row>
    <el-row class="margin-top-20">
      <el-col :span="4">
        <el-button
          v-if="hasSource(GLOBAL.config.sourceName.delete)"
          :size="GLOBAL.config.systemSize"
          @click="deleteBatch"
          type="danger"
          icon="el-icon-delete"
        ...
        <el-button
          v-if="hasSource(GLOBAL.config.sourceName.add)"
          :size="GLOBAL.config.systemSize" @click="addData" type="primary"
    ...
    <el-tabs v-model="tabSelect" @tab-click="tabChange">
      ...
      <el-tab-pane label="在办" name="working">
        ...
        >
          ...
          <el-table-column prop="registrateTime" label="登记日期">
          ...
          <el-table-column prop="riskType" label="风险类型" align="center">
            <template slot-scope="scope">
              ...
              scope.row.riskType,
              GLOBAL.config.dictionaryPre + GLOBAL.config.dictionary.riskType)
              }}
            </template>
          </el-table-column>
          ...
          <el-table-column prop="servicePersonal" label="服务人员" align="center">
            <template slot-scope="scope">
              ...
              <i v-if="hasSource(GLOBAL.config.sourceName.replace)"
                 class="el-icon-s-operation" style="cursor: pointer; color: #409EFF;" title="修改服务人员"
      ...
      <el-tab-pane label="结案" name="closed">
        ...
        >
          ...
          <el-table-column prop="servicePersonal" label="服务人员" align="center">
            <template slot-scope="scope">
              {{scope.row.servicePersonal}}
              <i v-if="hasSource(GLOBAL.config.sourceName.replace)" class="el-icon-s-operation" style="cursor: pointer; color: #409EFF;" title="修改服务人员"
                 @click="modifyServicePersonal(scope.row)"></i>
            </template>
          </el-table-column>
          <el-table-column prop="updateTime" label="更新时间" width="140">
  ...
  export default {
    ...
    data () {
      return {
        ...
        loading: false,
        timer: null,
        sourceList: JSON.parse(unescape(localStorage.getItem(this.GLOBAL.config.sourceList)))
          ?
          JSON.parse(unescape(localStorage.getItem(this.GLOBAL.config.sourceList))).filter(item => !item.value.indexOf(this.GLOBAL.config.modeName.risk))
          :
          null
      }
    },
    ...
    methods: {
      ...
      getSource: function (rowData) {
        ...
        let tempList = [{icon: 'el-icon-view', title: '查看', method: 'handleView'}]
        if (this.hasSource(this.GLOBAL.config.sourceName.update)) {
          tempList.push({icon: 'el-icon-edit', title: '编辑', method: 'handleEdit'})
        }
        if (this.hasSource(this.GLOBAL.config.sourceName.delete)) {
          tempList.push({icon: 'el-icon-delete', title: '删除', method: 'handleDelete'})
        }
        if (this.hasSource(this.GLOBAL.config.sourceName.remind)) {
          tempList.push({icon: 'el-icon-bell', title: '提醒', method: 'handleRemind'})
        }
        return tempList
      },
      hasSource: function (targetValue) {
        let sourceInfo = this.sourceList
          ?
          this.sourceList.filter(item => item.value.indexOf(targetValue) >= 0)
          :
          []
      ...
      getTableData: function (initPageFlag) {
        ...
        searchParams.eventStatus = selectTab
      ...
      tabChange: function (target) {
        let businessUnit = target.name
        // 列表是否重新加载
        !this.tableData[businessUnit].length && this.getTableData('init')
        // 删除按钮是否启用
        if (this.deletebatchList[businessUnit] &&
          this.deletebatchList[businessUnit].length > 0) {
      ...
      tableRowClassName ({row}) {
        return row.riskGrade ? (row.riskGrade + '-row') : ''
      },
      // 列表数据导出
