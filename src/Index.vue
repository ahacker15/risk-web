    ...
    <el-container>
      ...
      <el-header>
        <el-row>
          ...
          <el-col :span="20">&nbsp;
            <span class="user-name">{{getUserInfo()?getUserInfo():'黄智强'}}</span>
          </el-col>
          <el-col :span="2">
            <div class="user-area">
      ...
      <!--主体-->
      <el-main>
        <caseNone ref="caseNone" v-if="selectMenu === GLOBAL.config.modeName.noneLaw"/>
        <caseLaw ref="caseLaw" v-if="selectMenu === GLOBAL.config.modeName.cases"/>
        <risks ref="risks" v-if="selectMenu === GLOBAL.config.modeName.risk"/>
      </el-main>
...
<script>
  // 开发页面
  import caseNone from './view/noneLaw/CaseNone'
  import caseLaw from './view/cases/Case'
  import risks from './view/risk/Risks'
  ...
  export default {
    ...
    data () {
      return {
        systemName: '法务风险管理系统',
        systemSubName: 'Legal Risk Management System',
        footer: 'Copyright © 1999 - 2019 科大讯飞股份有限公司 版权所有',
        headPic: 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png',
        isCollapse: false,
        // 默认选中菜单
        selectMenu: JSON.parse(unescape(localStorage.getItem(this.GLOBAL.config.authList))).filter(item => item.url)[0].url,
        menuList: JSON.parse(unescape(localStorage.getItem(this.GLOBAL.config.authList))).filter(item => item.url)
      }
    },
    components: {
      caseNone,
    ...
    methods: {
      ...
      getUserInfo: function () {
        function g () {
          if (JSON.parse(unescape(localStorage.getItem('userInfo')))) {
            return JSON.parse(unescape(localStorage.getItem('userInfo'))).name
          }
        }
      ...
      initDictionary: function () {
        let dictionaryData = [
          ...
          {'name': '未定义', 'code': 'undetermined', sort: 4, 'parentCode': 'riskGrade'},
